...
<details>
<summary>항목 54 : TR1을 포함한 표준 라이브러리 구성요소와 편안한 친구가 되자</summary>

최초 상정된 표준 C++ 라이브러리는 STL, iostream, 로케일 등 + C89 표준 라이브러리

TR1이 도입되면서 추가된 것은 스마트포인터, 일반화 함수포인터, 해시 기반 컨테이너, 정규 표현식 등

TR1 구현을 구할 수 있는 자료처 중 하나는 부스트

</details>

<details>
<summary>항목 55 : 부스트를 늘 여러분 가까이에</summary>

부스트는 오픈 소스 C++ 라이브러리 개발하는 모임 웹사이트
